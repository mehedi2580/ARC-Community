...
import {
  ...
  Star, Users, Settings, Zap, Edit3, BadgeCheck, LogOut
} from "lucide-react";
  ...
  return (
    <aside className="hidden lg:flex flex-col fixed left-0 top-0 h-full w-64 xl:w-72 glass border-r border-white/[0.07] z-50 py-6 px-4">
      ...
      <button
        ...
        className="flex items-center justify-center gap-2 w-full py-3 px-4 bg-accent hover:bg-accent-light rounded-xl font-bold text-white text-sm transition-all duration-200 hover:shadow-accent hover:-translate-y-0.5 active:translate-y-0 mb-4"
      >
        <Edit3 size={16} />
        Cast
      </button>
      ...
      {/* Profile */}
      <Link href="/profile" className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-surface2 transition-colors group cursor-pointer mb-1">
        <div className="relative">
          <div className="w-9 h-9 rounded-full bg-gradient-to-br from-accent to-cyan flex items-center justify-center text-white font-bold text-sm border-2 border-accent/30">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-muted font-mono truncate">@{user.username}</p>
        </div>
      </Link>
      ...
      <button
        onClick={logout}
        className="flex items-center gap-3 px-3 py-2 rounded-xl text-xs font-semibold text-muted hover:text-red-400 hover:bg-red-400/5 transition-colors"
      >
        <LogOut size={14} />
        <span className="font-mono truncate text-[10px]">{user.walletAddress.slice(0,6)}…{user.walletAddress.slice(-4)}</span>
      </button>
    </aside>
  );
}
