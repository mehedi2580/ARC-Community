...
import { X, Image, Hash, AtSign, Smile } from "lucide-react";
  ...
  return (
    <>
      ...
      <div className="fixed inset-x-4 top-1/2 -translate-y-1/2 z-50 max-w-lg mx-auto">
        <div className="bg-surface border border-white/[0.1] rounded-2xl shadow-card overflow-hidden animate-slide-up">
          {/* Header */}
          ...
          <div className="p-4 flex gap-3">
            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-accent/60 to-cyan/60 flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
              {currentUser.name[0]}
            </div>
              ...
              <div className="relative w-7 h-7">
                <svg className="w-7 h-7 -rotate-90" viewBox="0 0 28 28">
                  ...
                  <circle
                    ...
                    strokeDashoffset={`${2 * Math.PI * 10 * (1 - Math.min(content.length / charLimit, 1))}`}
                    strokeLinecap="round"
                    className="transition-all"
                  />
                </svg>
                {remaining <= 50 && (
                  <span className={`absolute inset-0 flex items-center justify-center text-[9px] font-bold ${remaining < 0 ? "text-red-400" : "text-muted"}`}>
                    {remaining}
                  </span>
                )}
              </div>
